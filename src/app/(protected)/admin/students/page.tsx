...
export default async function AdminStudentsPage() {
  await requireSession('/admin/students')
  const client = createServiceClient()
  const { tenantId } = await resolveAdminTenantContext(client)
  const members = await listMembersByRole('students')
  ...
  const content = await AdminOnly({
    children: (
      <main className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 pb-20 md:px-8">
        <TopBar
          title="Students"
          subtitle="Track learner access, promote high performers, and maintain a healthy classroom community."
        />
        <TabBarMobile tabs={mobileTabs} />
        {tenantId ? (
          <StudentsClient tenantId={tenantId} members={members} />
        ) : (
          <p className="rounded-2xl border border-purple-800/40 bg-[#0b0615] p-6 text-sm text-purple-200">
            Select an institute before managing students.
          </p>
        )}
      </main>
    ),
  ...
  if (!content) {
    return (
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-4 px-6 py-16 text-purple-100">
        ...
        <p className="text-sm text-purple-200">Switch to admin view or request elevated permissions to manage students.</p>
      </main>
    )
  }
